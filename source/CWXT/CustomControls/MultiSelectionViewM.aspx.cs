...
{
	/// <summary>
	/// MultiSelectionViewM 的摘要说明。
	/// </summary>
	public partial class MultiSelectionViewM : EnterpriseWebsite.WebUI.ScrollPage
	{
		protected Microsoft.Web.UI.WebControls.ToolbarButton btnCloseWindow;
		protected Microsoft.Web.UI.WebControls.ToolbarButton btnClear;
		protected Microsoft.Web.UI.WebControls.ToolbarButton btnQuery;
		protected Microsoft.Web.UI.WebControls.ToolbarButton btnSelectAll;
		protected CustomControls.CustomPaging ucCustomPaging;
		protected CustomControls.QueryProvider ucQueryProvider;
		...
		private void AppendServerEvents()
		{
			...
			this.btnSelectAll.ButtonClick += new Microsoft.Web.UI.WebControls.ToolbarItemEventHandler(btnSelectAll_ButtonClick);
		}
		...
		#region 选中全部
		private bool btnSelectAll_ButtonClick(object sender, EventArgs e)
		{
			this.BindGrid1();
            Page.ClientScript.RegisterStartupScript(this.GetType(), "BaseScript", 
				string.Format("<script>ClientInit();InitCheckbox();SetControlValues('{0}','{1}');SetControlTexts('{2}','{3}');window.close();</script>", 
				this.valueControlID, this.strValues, this.textControlID, this.strTexts));
			return false;
		}
