...
namespace BusinessRule.JHSY
{
	...
	public class CWBirthInfo
	{
		...
		/// <returns>结果集</returns>
		public DataTable GetCWBirthInfoList(out int totalCount, int pageSize, int pageIndex, 
			Common.OrderByType obType, BusinessFilter subfilter)
		{
			Wicresoft.Session.Session session = new Wicresoft.Session.Session();
			BusinessObjectCollection boc = new BusinessObjectCollection("CWBirthInfo");
		...
		/// <param name="subfilter">筛选条件对象</param>
		/// <returns>结果集</returns>
		public DataTable GetCWBirthInfoListForExport(int pageSize, int pageIndex, BusinessFilter subfilter)
		{
			Wicresoft.Session.Session session = new Wicresoft.Session.Session();
			BusinessObjectCollection boc = new BusinessObjectCollection("CWBirthInfo");
			boc.SessionInstance = session;
