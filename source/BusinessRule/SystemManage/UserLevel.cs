...
namespace BusinessRule.SystemManage
{
	...
	public class UserLevel
	{
		...
		/// <returns>结果集</returns>
		public DataTable GetUserLevelList(out int totalCount, int pageSize, int pageIndex, 
			Common.OrderByType obType, BusinessFilter subfilter)
		{
			Wicresoft.Session.Session session = new Wicresoft.Session.Session();
			BusinessObjectCollection boc = new BusinessObjectCollection("UserLevel");
			boc.SessionInstance = session;
			BusinessFilter filter = new BusinessFilter("UserLevel");
